...
## the reprobuild test graph (which also passes its own `--nimcache:`), CI and
## a bare `nim c` alike, whatever the working directory.
...
## Nothing in this repository passes `--skipParentCfg` today.  Its tests run only through
## the typed test list in `repro.nim`, so there is no separate check test;
## `nim dump <file>` shows where a given compile will put its cache.
##
